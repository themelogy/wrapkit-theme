...
class HeaderMenuWithIconPresenter extends Presenter
{
    /**
     * {@inheritdoc }.
     */
    ...
    {
        $link = '<li'.$this->getActiveState($item).'><a class="c-link'.$this->getActiveState($item, ' current').'" href="'.$item->getUrl().'" '.$item->getAttributes().'>';
        if($item->getIcon('', false)) {
            $link .= '<span aria-hidden="true" class="'.$item->getIcon('', false).' main-menu-icon theme-color"></span>';
        }
